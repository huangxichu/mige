  ...
  <el-container style="height: 100%" id="dept_table">
    <el-header id="dept_head" style="margin-top: 10px!important;height: 40px!important;">
      <el-form ref="search"  :model="search" label-width="80px" style="width: 100%;">
        <el-row>
          <el-col :span="4">
            <el-input v-model="search.deptName" placeholder="部门名称" size="mini"></el-input>
          </el-col>
          <el-col :span="4">
    ...
    <el-dialog title="部门" :visible.sync="dialogEditVisible"
               :close-on-press-escape="true"
               :close-on-click-modal="false"
               :show-close="true" width="600px">
      <el-form :model="form" label-width="100px" :rules="rules" ref="editFrom">
        <el-form-item label="部门名称" prop="deptName">
          <el-input v-model="form.deptName" auto-complete="off"  placeholder="请输入部门名称"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogEditVisible = false">取 消</el-button>
        <el-button type="primary" @click="saveDepartment_2('editFrom')">确 定</el-button>
      </div>
    </el-dialog>
  </el-container>
</template>
...
<script>
  import {getDepartment,saveDepartment,deleteDepartment} from './api'
  ...
  export default {
    ...
    methods: {
      ...
      saveDepartment_2(formName){
        let _this = this
        this.$refs[formName].validate((valid) => {
          if (valid) {
            saveDepartment(_this.form,_this).then(function(res) {
              if(res.status == '200'){
                if(res.data.code === '0'){
      ...
      gotoAdd() {
        ...
        _this.form={id:'',deptName:'',companyCode:''}
      ...
      gotoEdit(row) {
        ...
        _this.form={id:row.id,deptName:row.deptName,companyCode:row.companyCode,status:row.status}
        //this.$router.push({name:'article-edit'})
        _this.dialogEditVisible = true;
      },
      upStatus(row,status){
        let _this = this
        row.status = status;
        saveDepartment({id:row.id,deptName:row.deptName,companyCode:row.companyCode,status:row.status},_this).then(function(res) {
          if(res.status == '200'){
            if(res.data.code === '0'){
              _this.dialogEditVisible = false
              _this.loadData({},1,10)
      ...
      removeRow(row){
        ...
        _this.$confirm('此操作将永久删除, 是否继续?', '提示', {
          ...
          deleteDepartment({id:row.id},_this).then(function(res) {
            if(res.status == '200'){
              if(res.data.code === '0'){
                _this.dialogEditVisible = false
                _this.loadData({},1,10)
      ...
      loadData(params,page,rows){
        ...
        _this.page = page
        _this.rows = rows
        getDepartment(params,_this).then(function(res) {
          if(res.status == '200'){
            if(res.data.code === '0'){
